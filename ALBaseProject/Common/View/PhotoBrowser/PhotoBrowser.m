                   ...
                   delegate:(id<PhotoBrowserDelegate>)delegate
{
    ...
    if (self)
    {
        ...
        _pageControl.pageIndicatorTintColor = RGBA(255, 255, 255, 0.3);
...
- (void)show
{
    ...
    keyWindow.windowLevel = UIWindowLevelStatusBar+10.0f;
...
- (void)downloadContentWithIndex:(NSInteger)index
{
    ...
    if(!image)
    {
        ...
        [[SDWebImageManager sharedManager] loadImageWithURL:url options:SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            ...
            if (finished && image)
            {
                [self updateImageViewForIndex:index image:image];
...
- (void)initImageView:(NSInteger)i
{
...
- (void)loadImageView:(NSInteger)i
{
    if (i < 0 || i >= [_bigImageInfos count])
    {
        return;
        ...
        else if ([smallImageInfo isKindOfClass:[NSString class]])
        {
            ...
            [[SDWebImageManager sharedManager] loadImageWithURL:url options:SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                ...
                if (finished)
                {
                    if (image)
                    {
                        [self updateImageViewForIndex:i image:image];
                    }
...
- (void)longPressEvent:(UIGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan)
    {
        ...
        {
            LZHAlertView *alertView = [LZHAlertView createWithTitleArray:@[@"取消",@"保存"]];
            alertView.contentLabel.text = @"是否保存图片";
            __weak PhotoBrowser *weakSelf = self;
            __weak LZHAlertView *weakAlert = alertView;
            [alertView setBlock:^(NSInteger index, NSString *title) {
                if (index == 1) {
                    [weakSelf saveCurrentImage];
                }
                [weakAlert hide];
            }];
            [alertView show];
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if (!error) {
        dispatch_async(dispatch_get_main_queue(), ^(void){
            LZHAlertView *alertView = [LZHAlertView createWithTitleArray:@[@"确定"]];
            alertView.contentLabel.text = @"保存成功";
            //__weak PhotoBrowser *weakSelf = self;
            __weak LZHAlertView *weakAlert = alertView;
            [alertView setBlock:^(NSInteger index, NSString *title) {
            ...
            [alertView show];
            //[AlertHelper showAlertWithTitle:@"保存成功"];
    ...
    else
    {
        dispatch_async(dispatch_get_main_queue(), ^(void){
            LZHAlertView *alertView = [LZHAlertView createWithTitleArray:@[@"确定"]];
            alertView.contentLabel.text = @"保存失败";
            //__weak PhotoBrowser *weakSelf = self;
            __weak LZHAlertView *weakAlert = alertView;
            [alertView setBlock:^(NSInteger index, NSString *title) {
